...
export default function HomePage() {
  ...
  const { data: homeData, loading } = useFetch(getHome, [])
  ...
  const completedList = (home?.completed?.animeList || []).map(normalizeAnimeItem)

  const heroAnime = ongoingList[0] || completedList[0] || null

  // Build poster lookup from already-loaded data
  const posterLookup = {}
  ongoingList.forEach((a) => { if (a.id) posterLookup[a.id] = a.poster })
  completedList.forEach((a) => { if (a.id) posterLookup[a.id] = a.poster })

  // Fetch missing posters for history items
  useEffect(() => {
    const missing = history.filter(
      (item) => !posterLookup[item.animeId] && !posterCache[item.animeId]
    )
    const uniqueIds = [...new Set(missing.map((m) => m.animeId).filter(Boolean))]
    if (uniqueIds.length === 0) return

    uniqueIds.forEach(async (animeId) => {
      try {
  ...
  const historyList = history.map((item) => ({
    ...
    poster: item.animePoster || posterLookup[item.animeId] || posterCache[item.animeId] || "",
    episodeLabel: item.episodeNumber ? `Eps ${item.episodeNumber}` : "Lanjut",
  }))

  const handleSelect = (id) => { if (id) navigate(`/anime/${id}`) }
  const handleGenre = (slug) => { if (slug) navigate(`/genre/${slug}`) }
  const handleHistorySelect = (slug) => { if (slug) navigate(`/episode/${slug}`) }
  ...
  const scrollHistory = (dir) => {
    ...
    historyRowRef.current.scrollBy({ left: dir === "left" ? -amount : amount, behavior: "smooth" })
  }
      ...
      <Container className="pt-8">
        {historyList.length > 0 && (
          <section className="mb-10">
            <h2 className="mb-4 text-xl font-bold text-text-primary">Lanjut Tonton</h2>
            <div className="group relative">
              ...
              <div ref={historyRowRef} className="flex gap-4 overflow-x-auto scroll-smooth pb-2 [scrollbar-width:none]">
                {historyList.map((item) => (
                  <div key={item.id} className="w-[160px] flex-shrink-0 sm:w-[180px]">
        ...
        <AnimeRow title="Ongoing" animeList={ongoingList} onSelect={handleSelect} />

        <AnimeRow title="Completed" animeList={completedList} onSelect={handleSelect} />
